...
# CLIENT SECRET
# 9B8zbIRl-iUSUPK8A9cbXYlQRK4cKU2k
...
# chunk ... 와 intents 설정시 유튜브 음악 재생이 안 되는 버그로 인해 잠시 막아 둠
# @bot.command(name='명령추가')
# async def slut_TJ(ctx):
#     msg = '나한테 무슨 명령을 추가할건데? 60초 안에 ㄱㄱ'
#     await ctx.send(msg)
#
#     print(f'({now}) {ctx.message.author} [{ctx.message.author.id}] : {ctx.message.content}')
#
#     def check(react_msg):
#         # 메시지가 비어있지 않고 동일한 채널에서 메시지를 보냈으며 메시지 작성자가 동일할 때
#         return react_msg != "" and react_msg.channel == ctx.channel and react_msg.author == ctx.author
#
#     try:
#         react_msg = await bot.wait_for('message', timeout=60.0, check=check)
#     except asyncio.TimeoutError:
#         await ctx.send(':middle_finger: 시간 지남 ㅅㄱ')
#     else:
#         await ctx.send('{.author} ... 확인'.format(react_msg))
#
#         to_id = 432499017071722497
#
#         # 태진 id : 432499017071722497
#         # 진수 id : 354957025593262081
#         # 병진 id : 389019613427400715
#         # 승현 id : 675723907625385984
#
#         # 메시지 보낸 사람의 아이디 얻기 message.author.id
#         author = ctx.guild.get_member(to_id)
#         msg = f'{react_msg.author}로부터 명령이 들어왔습니다.\n...\n{react_msg.content}'
#
#         # author가 아무런 객체를 넘겨받지 못 했을 경우.
#         if author is None:
#             await ctx.send('유태진의 id를 찾을 수 없습니다.')
#         else:
#             await author.send(msg)
#
#         print(f'({now}) {react_msg.author} [{react_msg.author.id}] : {react_msg.content}')
...
if __name__ == "__main__":
    # TOKEN 값을
    token = 'ODAxODAwMjA4MTc0Mjg0ODQw.YAl8og.NuZfR_DniNWxL0OOsy2uIUZieDI'
    bot.run(token)